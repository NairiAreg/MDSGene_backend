...
async def process_to_excel(
    ...
    try:
        ...
        if excel_path.exists():
            ...
            # Проверка и обновление существующей записи
            for _, new_row in df_new_rows.iterrows():
                ...
                if condition.any():
                    df_existing.loc[condition, new_row.index] = new_row.values
                else:
                    df_existing = pd.concat([df_existing, pd.DataFrame([new_row])], ignore_index=True)

            df_final = df_existing
        else:
            df_final = df_new_rows
...
def generate_gene_url(data):
    """
    Generate URLs for exported gene data in the format http://url/mdsgene/genes/gene-disease
    ...
    try:
        ...
        # Extract gene-disease pairs from the data
        for patient in data:
            ...
            # Create a unique key for this gene-disease pair
            pair_key = f"{gene}-{disease}"

            # Create URL for this pair
            gene_disease_pairs[pair_key] = f"{base_url}/genes/{gene}-{disease}"
